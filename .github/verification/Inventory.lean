...
private def audit (inputs : Inputs) (outputDir : System.FilePath) : CoreM AuditResult := do
  ...
  for (name, info, origin) in selected do
    -- Lean 4.35.0-rc2's public API consults imported precomputed axiom dependencies before recursing.
...
private def writeResults (inputs : Inputs) (env : Environment)
  ...
  let summary := Json.mkObj [
    ...
    ("axiomCollectionMethod", Json.str "Lean.collectAxioms once per selected declaration; Lean 4.35.0-rc2 imported precomputed dependencies avoid repeated closure walks; private roots may use its fallback traversal"),
